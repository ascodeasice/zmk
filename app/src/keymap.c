...
// Needed to ensure our settings storage has a fixed size for entries
#define MAX_BEHAVIOR_DEV_NAME 20

struct zmk_behavior_binding_setting {
    char behavior_dev[MAX_BEHAVIOR_DEV_NAME];
    uint32_t param1;
    uint32_t param2;
} __packed;
    ...
    for (int l = 0; l < ZMK_KEYMAP_LAYERS_LEN; l++) {
        ...
        for (int kp = 0; kp < ZMK_KEYMAP_LEN; kp++) {
            if (pending[kp / 8] & BIT(kp % 8)) {
                ...
                strncpy(binding_setting.behavior_dev, binding->behavior_dev,
                        sizeof(binding_setting.behavior_dev));

                char setting_name[20];
                sprintf(setting_name, "keymap/l/%d/%d", l, kp);

                settings_save_one(setting_name, &binding_setting,
...
static int keymap_handle_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg) {
    ...
    if (settings_name_steq(name, "l", &next) && next) {
        ...
        if (len != sizeof(struct zmk_behavior_binding_setting)) {
            LOG_ERR("Invalid binding setting size (got %d expected %d)", len,
                    sizeof(struct zmk_behavior_binding_setting));
            return -EINVAL;
        }
        ...
        struct zmk_behavior_binding_setting binding_setting;
        int err = read_cb(cb_arg, &binding_setting, sizeof(struct zmk_behavior_binding_setting));

        const struct device *behavior = behavior_get_binding(binding_setting.behavior_dev);

        if (!behavior) {
            LOG_WRN("Loaded device %s from settings but no device found by that name",
                    binding_setting.behavior_dev);
        }
